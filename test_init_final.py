							...
							if basicSetting[2] != '0' and basicSetting[22] != '0' and bossFlag[i] == True and bossFlag0[i] == True and bossMungFlag[i] == True :
								if int(basicSetting[17]) <= bossMungCnt[i] and int(basicSetting[17]) != 0:
									...
									################ 멍 보스 ################
										...
										tmp_bossTimeString[i] = bossTimeString[i] = nextTime.strftime('%H:%M:%S')
										tmp_bossDateString[i] = bossDateString[i] = nextTime.strftime('%Y-%m-%d')
										await self.bot.get_channel(channel).send("```" + bossData[i][0] + ' pass.```')
										embed = discord.Embed(
											description= '```다음 ' + bossData[i][0] + ' ' + bossTimeString[i] + '입니다.```',
											color=0xff0000
											)
...
class mainCog(commands.Cog): 
	...
	async def menu_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			command_list += '[보스명]kut 또는 [보스명]kut 0000, 00:00\n'  
			command_list += '[보스명] kut 또는 [보스명] kut 0000, 00:00\n'   
			...
			embed.add_field(
					name="----- 추가기능 -----",
					value= '```- [보스명]kut/pass/예상  [할말] : 보스시간 입력 후 빈칸 두번!! 메모 가능\n- [보스명]kut 명령어는 초성으로 입력가능합니다.\n  ex)' + bossData[0][0] + 'kut => ' + convertToInitialLetters(bossData[0][0] +'kut') + ', ' + bossData[0][0] + ' kut => ' + convertToInitialLetters(bossData[0][0] +' kut') + '```'
					)
	...
	async def on_message(self, msg):
		...
		if chflg == 1 :
			if self.get_channel(basicSetting[7]).id == msg.channel.id:
				...
				for command_str in ["kut", "pass", "예상", "삭제", "메모", "카톡켬", "카톡끔"]:
				...
				for i in range(bossNum):
					################ 보스 컷처리 ################ 
					if message.content.startswith(bossData[i][0] +'kut') or message.content.startswith(convertToInitialLetters(bossData[i][0] +'kut')) or message.content.startswith(bossData[i][0] +' kut') or message.content.startswith(convertToInitialLetters(bossData[i][0] +' kut')):
						if hello.find('  ') != -1 :
